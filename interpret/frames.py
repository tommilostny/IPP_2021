...
def set_variable(instr_name:str, instr_order:int, type:str, var:str, value, def_check:bool=False):
    global global_frame, local_frames, temporary_frame
    if type == "var":
        frame, _, varname = var.partition("@")

        if frame == "GF" and (not def_check or varname not in global_frame.keys()):
            global_frame[varname] = value

        elif frame == "LF" and len(local_frames) and (not def_check or varname not in local_frames[-1].keys()):
            local_frames[-1][varname] = value

        elif frame == "TF" and temporary_frame is not None and (not def_check or varname not in temporary_frame.keys()):
